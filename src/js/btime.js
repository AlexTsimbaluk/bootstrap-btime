...
+function ($) {
    ...
    Btime.prototype.checkBtime = function() {
        ...
        if (!_this.$element.val().match(/^\d{2}:\d{2}/)) {
            bres.val('');
            return;
        }

        var hours = parseInt((_this.$element.val().split(':'))[0].replace(/^0/, ''), 10);
        var minute = parseInt((_this.$element.val().split(':'))[1].replace(/^0/, ''), 10);

        if (hours > 23) {
            _this.$element.val( '23:' + (_this.$element.val().split(':'))[1] );
            return _this.checkBtime();
        }

        if (minute > 59) {
            _this.$element.val( (_this.$element.val().split(':'))[0] + ':59' );
            return _this.checkBtime();
        }

        var res_date = $('#' + _this.config.dateSelector).val() + ' ' + _this.$element.val() + ':00 ' + _this.rfc_zone( now );
        if (res_date > _this.rfc_datetime( end )) {
            _this.$element.val(user_time( end ));
        } else if (res_date < _this.rfc_datetime( first )) {
            _this.$element.val(_this.user_time( first ));
        }

        res_date = $('#' + _this.config.dateSelector).val() + ' ' + _this.$element.val() + ':00 ' + this.rfc_zone( now );

        bres.val(res_date);
        console.log(bres.val());

        console.log('checkBtime:end');
    }

    // По таймеру речекаем время при переходе на следующий день
    Btime.prototype.checkBtimeAuto = function() {

        console.log('checkBtimeAuto:begin');
